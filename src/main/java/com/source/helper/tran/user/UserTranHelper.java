package com.source.helper.tran.user;
...
public class UserTranHelper {
	...
	public String generateUserName(String email, String mobileNo, String generatedBy) {
		if ("email".equalsIgnoreCase(generatedBy)) {
			String[] parts = email.split("@");
			if (parts.length == 2) {
				String username = parts[0];
				return username;
			}
		} else if ("mobile".equalsIgnoreCase(generatedBy)) {
			return mobileNo;
		}
	...
	public String getEmailVerifiedMessage(UserMaster user, String type) {
		...
		if (type.equalsIgnoreCase("success")) {
			String emailContent = "<!DOCTYPE html>\n" + "<html>\n" + "<head>\n"
					...
					+ "    <p style=\"text-align: center;\" >Dear <b>" + user.getFirstName() + "</b>,Your account with "
					+ appName
					+ " has been verified successfully! .You can now log in and start using our services.</p>\n"
					+ "</body>\n" + "</html>";
			return emailContent;
		} else if (type.equalsIgnoreCase("failed")) {
			String emailContent = "<!DOCTYPE html>\n" + "<html>\n" + "<head>\n"
					+ "    <title>Account Verification Failed</title>\n" + "</head>\n" + "<body>\n"
					+ "    <p style=\"text-align: center;\">Dear <b>" + user.getFirstName()
					+ "</b>,We regret to inform you that we were unable to verify your account with " + appName
					+ ".If you believe this is in error, please contact our support team .</p>\n" + "</body>\n"
					+ "</html>";
			return emailContent;
		}
